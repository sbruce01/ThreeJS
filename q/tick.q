...
// Initialising tp logging table 
.tp.logging:`trade`quote`aggregation!(0;0;0);

\l tick/u.q
\d .u
ld:{if[not type key L::`$(-10_string L),string x;.[L;();:;()]];i::j::-11!(-2;L);if[0<=type i;-2 (string L)," is a corrupt log. Truncate to length ",(string last i)," and restart";exit 1];hopen L};
tick:{init[];@[;`sym;`g#]each t;d::.z.D;if[l::count y;L::`$":",y,"/",x,10#".";l::ld d]};
...
if[system"t";
 ...
 upd:{[t;x]
 if[t = `trade;.debug.tx:(t;x)];
 if[not -16=type first first x;if[d<"d"$a:.z.P;.z.ts[]];a:"n"$a;x:$[0>type first x;a,x;(enlist(count first x)#a),x]];
 t insert x;if[l;l enlist (`upd;t;x);j+:1];}];

if[not system"t";system"t 60000"; // Changed to minute check/logging
 // Adding logging
 .z.ts:{ts .z.D;
    0N!"Ingestion Count:";
    0N!.tp.logging;
    0N!"Tables and Subscription Ports:";
    0N!(key .u.w)!{{x"\\p"}each x} each {first each x} each value .u.w;
 };
 ...
 $[98h~type x;
   [
      pub[t;x];if[l;l enlist (`upd;t;x);i+:1];
   ];
   [
